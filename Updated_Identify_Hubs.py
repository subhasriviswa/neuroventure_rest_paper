...
def main(argv=sys.argv):
    ...
    mult_mat_path = '/Users/venturelab/Downloads/iamdamion-Demeter_etal_2023-735a8f6/COMBINED333_LR_Distance_MULTIPLICATION_MASK.csv'
    gordon_parcel_path = '/Users/venturelab/Downloads/iamdamion-Demeter_etal_2023-735a8f6/Parcels_LR.dlabel.nii'
    threshold_list = [0.003, 0.004, 0.005, .01, .015, .02, .025, .03, .035, .04, .045, .05]

    def load_ztrans_mat(ztrans_mat_path):
    # Load the z-transformed correlation matrix directly from the file
        ztrans_mat = np.loadtxt(ztrans_mat_path, delimiter=',')
        ztrans_mat = ztrans_mat.astype(float)
        return ztrans_mat
    ...
    def run_infomap(subid, ready_zmat):
        ...
        for thresh in threshold_list:  
            ...
            nx_graph = nx.from_numpy_array(thresh_mat)
